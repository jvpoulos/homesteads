...
%Imputation and matrix completion are two widely-used methods for reconstructing missing data. Imputation methods reconstruct missing data by capturing synchronous relationships among observed values at the same time across different outcomes but ignore the temporal relationships within outcomes. Commonly used imputation methods include prediction model imputation, which replaces missing values with the predicted values from a regression of $\mathbf{Y_{\mathrm{mis}}}$ on $\mathbf{Y_{\mathrm{obs}}}$, and data analysis techniques such as principal component analysis (PCA) \citep{ilin2010practical}. These methods rely on modeling assumptions about the nature of the data-generating process and assume that the probability of missingness depends only on the observed data, $f(\mathbf{M} | \mathbf{Y}, \phi) = f(\mathbf{M} | \mathbf{Y_{\mathrm{obs}}}, \phi)$ for all $\mathbf{Y_{\mathrm{mis}}}, \phi$. This missing at random (MAR) assumption is easily violated when considering the problem of counterfactual prediction due to the dependency structure in $\mathbf{M}$. 

Matrix completion methods can accurately predict $\mathbf{Y_{\mathrm{mis}}}$ by solving a convex optimization problem via nuclear norm minimization (NNM), when the full matrix $\mathbf{Y}$ is low-rank and relatively few entries are observed \citep[e.g.,][]{candes2009exact,candes2010matrix}.\footnote{Low-rank matrices arise in the present context when only a few factors contribute to the outcomes. \citet{candes2009exact} observe that $\mathbf{Y_{\mathrm{obs}}}$ cannot be mostly equal to zero.} These methods exploit relationships within and across outcomes by treating $\mathbf{Y}$ as static and typically assume the missing values in $\mathbf{Y}$ are sampled uniformly at random. Incorrect assumptions about missing data can yield biased predictions in probabilistic models including regularized matrix factorization \citep{marlin2009collaborative}. 

The method of matrix completion via NNM (MC-NNM) proposed by \citet{athey2017matrix} allows for patterns of missing data in $\mathbf{Y}$ to have a time-series dependency structure that arise from simultaneous or staggered adoption. $\mathbf{Y}$ can be decomposed as $\mathbf{Y} = \mathbf{L^{*}} + \epsilon$, where it is assumed that the conditional expectation of the noise term $\epsilon$ given the matrix to be estimated, $\mathbf{L^{*}}$, is equal to zero. The MC-NNM estimator for $\mathbf{L^{*}}$ minimizes the sum of squared reconstruction errors $(Y_{it} - L_{it})$ for values with $M_{it} = 0$ via nuclear norm regularized least squares, with the regularization term chosen by cross-validation.\footnote{\citet{amjad2018robust} propose an alternative approach of approximating $\mathbf{L^{*}}$ via matrix estimation, and then using linear regression on the ``de-noised'' matrix, rather than relying on matrix norm regularizations.}\footnote{\citet{athey2017matrix} note that their estimator penalizes the errors for each observed value equally without regard to the fact that the probability of missingness (i.e., the propensity score), $\Pr (M_{it} = 1 | \mathbf{Y_{\mathrm{obs}}}, \phi)$, increases with $t$. The authors suggest weighting the loss function by the propensity score, which is similar to the importance weighting scheme proposed by \citet{cortes2008sample} to address the problem of covariate shift.} The algorithm for calculating the estimator iteratively replaces missing values with those recovered from a singular value decomposition (SVD) \citep{mazumder2010spectral}.\footnote{The root-mean squared error (RMSE) of the estimator is given by $\|\mathbf{L^{*}}-\hat{\mathbf{L}}\|_F / \sqrt{NT}$, where $F$ is the Fröbenius Norm. \citet{athey2017matrix} prove that, as $N$ and $T$ increase, the RMSE converges to zero when $\mathbf{L^{*}}$ is low-rank and the number of control units is sufficiently large.}

\subsection{Inference}

%. Define the counterfactual means for the treatment unit as
%For each time instance t ∈ [T ], compute the model uncertainty (variance) as
...
%	3 the number of (potential) control units, J, is often of the same order as T0, which leads to a need for some regularization
