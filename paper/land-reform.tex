...
\begin{table}[htbp]
	...
	\input{placebo-tests}
	\end{center}
	\footnotesize{\emph{Notes:} randomization $p$-values calculated by Equation (\ref{eq:pvalue}) corresponding to each permutation structure and value of $\tau$ and $q$. i.i.d. block $p$-values are calculated using $|\Pi| = 10,000$ permutations. Moving block $p$-values are based on $|\Pi| = T-1$ permutations.}
\end{table}
