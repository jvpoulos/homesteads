...
\begin{description}
	{\setlength\itemindent{1mm}
		\item[DID] DID estimator with unit and time fixed effects \citep{athey2018design}
...
\end{equation}
%
where $\left\{\bar{\alpha}^{o}_{t}\right\}_{t=T_0}^T$ is a trajectory of per-period average effects under the null. The test statistic suggested by \citet{chernozhukov2017exact} summarizes the trajectory of causal effects over $T_\star = T-T_0$ post-periods:
%
\begin{equation}
...
\text{H}_0: \frac{1}{\tau +1} \sum_{T_0 - \tau +1}^{T_0} \hat{\bar{\alpha}}_{t} = 0,
\end{equation}
%
for $\tau \in \left\{10, 20, 30\right\}$. Equation (\ref{h0-placebo}) is tested by the same procedure described in Section \ref{hyp-test}, treating $\left\{1, \ldots, T_0 - \tau\right\}$ as the pre-period and $\left\{T_0 - \tau +1, \ldots, T_0\right\}$ as the post-period.

Table \ref{placebo-tests} reports the average treatment effect over the placebo post-period and 95\% randomization confidence intervals for the null hypothesis test. We cannot reject the placebo null hypothesis for each of the three outcomes and three permutation structures, since all the intervals contain zero. This provides evidence that the estimator is consistent. 
...
\begin{table}[htbp]
	...
	\end{center}
	\footnotesize{Notes: Mean $\hat{\bar{\alpha}}_{t}$ corresponds to quantity on the left-hand-side of Equation (\ref{h0-placebo}). Values in brackets represent 95\% confidence intervals constructed by inverting the randomization test described in Section \ref{hyp-test} on $|\mathcal{A}_t| = 2,500$ null trajectories. Confidence intervals based on i.i.d. permutations are calculated using $|\Pi| = 1,000$ permutations.}
\end{table}
...
\section{DID estimates} \label{DID}

The proposed method estimates the impact of a discrete event on a continuous outcome. However, in this application a continuous form of treatment is available in the form of homestead entries aggregated from individual land patent records.

I estimate the following DID model, where the first difference is the change in patterns of state capacity before and after implementation of the HSA or SHA, and the second difference comes from the intensity of homestead entries across frontier states:
