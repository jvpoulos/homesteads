...
Imputation and matrix completion are two widely-used methods for reconstructing missing data. Imputation methods reconstruct missing data by capturing synchronous relationships among observed values at the same time across different outcomes but ignore the temporal relationships within outcomes. Commonly used imputation methods include prediction model imputation, which replaces missing values with the predicted values from a regression of $\mathbf{Y_{\mathrm{mis}}}$ on $\mathbf{Y_{\mathrm{obs}}}$. These methods rely on modeling assumptions about the nature of the data-generating process and assume that the probability of missingness depends only on the observed data, $f(\mathbf{M} | \mathbf{Y}, \phi) = f(\mathbf{M} | \mathbf{Y_{\mathrm{obs}}}, \phi)$ for all $\mathbf{Y_{\mathrm{mis}}}, \phi$. This missing at random (MAR) assumption is easily violated when considering the problem of counterfactual prediction due to the dependency structure in $\mathbf{M}$. 

Matrix completion methods can accurately predict $\mathbf{Y_{\mathrm{mis}}}$ by solving a convex optimization problem via nuclear norm minimization (NNM), when the full matrix $\mathbf{Y}$ is low-rank and relatively few entries are observed \citep[e.g.,][]{candes2009exact,candes2010matrix, mazumder2010spectral}.\footnote{Low-rank matrices arise in the present context when only a few factors contribute to the outcomes. \citet{candes2009exact} observe that $\mathbf{Y_{\mathrm{obs}}}$ cannot be mostly equal to zero.} These methods exploit relationships within and across outcomes by treating $\mathbf{Y}$ as static and typically assume the missing values in $\mathbf{Y}$ are MAR and sampled uniformly at random.\footnote{Incorrect assumptions about missing data can yield biased predictions in probabilistic models like regularized matrix factorization \citep{marlin2009collaborative}.} The method of matrix completion via NNM (MC-NNM) proposed by \citet{athey2017matrix} allows for dependency structures in $\mathbf{M}$ that arise from simultaneous or staggered adoption.%\footnote{\citet{athey2017matrix} remark that their estimator penalize the error terms for each unit-time observation equally without regard to the fact that the probability of missingness (i.e., the propensity score), $\Pr (M_{it} = 1 | \mathbf{Y_{\mathrm{obs}}}, \phi)$, increases with $t$. The authors show that the estimator can be modified by using a loss function weighted by the propensity score, which is a solution akin to the importance weighting scheme proposed by \citet{cortes2008sample} for the problem of covariate shift.}
...
%\citep{ilin2010practical}
% troyanskaya2001missing - SVD
% josse2012handling - (regularized) iterative PCA

%The variable selection property of L1-regularized linear models (i.e., Lasso regression) \citep{tibshirani2012strong} is particularly useful in settings where $\mathbf{Y}$ has dimensions $N \ll T$. Lasso tends to shrink all but one of the coefficients of correlated outcomes to zero.
