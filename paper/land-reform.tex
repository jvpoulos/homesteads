...
Y_{it} =  \gamma_i + \delta_t + \gamma \, d_{it} + \delta \, (d_{it} \cdot \text{homesteads}_{it}) + X_{ip}  + \epsilon_{it}. \label{eq:dd} 
\end{equation}

In this model, $\gamma_i$ and $\delta_t$ are state and year effects, respectively, and $X_{ip}$ is a matrix of unit-specific pre-intervention covariates included to control for parallel trends in pre-intervention agricultural productivity and railroad access to frontier lands. In the staggered adoption setting, entries in the treatment indicator $d_{it} = 1$ at $t \geq T_0$, where the initial exposure time $T_0$ varies across units. The continuous treatment exposure variable $\text{homesteads}_{it}$ measures the per-capita statewide sum of homestead entries in state $i$ and year $t$.
