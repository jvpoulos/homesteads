...
\begin{table}[htbp]
	...
	\input{placebo-tests}
	\end{center}
	\footnotesize{\emph{Notes:} randomization $p$-values calculated by Equation (\ref{eq:pvalue}) corresponding to each permutation structure and value of $\tau$ and $q$. i.i.d. block $p$-values are calculated using $|\Pi| = 50,000$ permutations. Moving block $p$-values are based on $|\Pi| = T-1$ permutations.}
\end{table}
...
In this model, $a_i$ and $b_t$ are vectors of unit and time fixed effects, respectively, included to control for unobserved heterogeneity. In the staggered adoption setting, entries in the missing value indicator $M_{i,t}$ assume the value of 1 at $t \geq T_0$, where the initial exposure time $T_0$ varies across units. The continuous treatment exposure variable $\text{homesteads}_{i, t}$ measures the per-capita statewide sum of homestead entries in state $i$ and year $t$.
