...
taxpc.did$treat <- NA
taxpc.did$treat[taxpc.did$state.abb %in% southern.pub] <- 1
...
taxpc1.data <- data.frame(subset(taxpc.did, !is.na(taxpc1) & year<1929, select=c('time','treat','did','taxpc1')))
colnames(taxpc1.data)<- c('time','treat','did','y')
...
taxpc2.data <- data.frame(subset(taxpc.did, !is.na(taxpc2) & year<1929, select=c('time','treat','did','taxpc2')))
colnames(taxpc2.data)<- c('time','treat','did','y')
