...
south.exppc.lstm.pred.treated <- read_csv(paste0(results.directory, "lstm/south-exppc/treated-gans/weights.10-0.100.hdf5-south-exppc-test.csv"), col_names = FALSE)
south.exppc.lstm.pred.control <- read_csv(paste0(results.directory, "lstm/south-exppc/control/weights.10-16.942.hdf5-south-exppc-test.csv"), col_names = FALSE)
...
south.exppc.lstm.mse <- error(forecast=south.exppc.control.forecast, true=south.exppc.control.true, method = "mse") # post-intervention MSE
...
lstm.south.exppc.CI.treated <- PermutationCI(south.exppc.control.forecast, south.exppc.control.true, south.exppc.t.stat, south.exppc.n.placebo, c.range=c(-8,8), np=20000, l=1000)
...
south.exppc.lstm.long$ymin[south.exppc.lstm.long$group=="Treated"] <- lstm.south.exppc.CI.treated[,1]
south.exppc.lstm.long$ymax[south.exppc.lstm.long$group=="Treated"] <- lstm.south.exppc.CI.treated[,2]
...
mean(south.exppc.lstm.long$value[south.exppc.lstm.long$variable=="X1"])/mean(south.exppc.y[(south.exppc.n.pre+1):nrow(south.exppc.y),]) # get mean % treatment effect
mean(south.exppc.lstm.long$ymin[south.exppc.lstm.long$variable=="X1"])/mean(south.exppc.y[(south.exppc.n.pre+1):nrow(south.exppc.y),])
mean(south.exppc.lstm.long$ymax[south.exppc.lstm.long$variable=="X1"])/mean(south.exppc.y[(south.exppc.n.pre+1):nrow(south.exppc.y),])
