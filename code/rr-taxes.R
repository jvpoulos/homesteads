...
rr.taxpc <- rr.taxpc[rr.taxpc$track2<=0.3,] # share max is 0.3
...
rr.taxpc.plot <- ggplot(rr.taxpc, aes(track2, value, color = factor(variable))) + 
  ...
  annotate("text", x = 0.2, y = 10, label = LmEq(gam(value ~ track2, data=subset(rr.taxpc, variable=="taxpc1"))), colour=wes_palette("FantasticFox")[3], size = 4, parse=TRUE) +
  geom_smooth(data=subset(rr.taxpc, variable=="taxpc2"),se=TRUE, colour=wes_palette("Moonrise3")[2],size=1) +
  annotate("text", x = 0.25, y = 20, label = LmEq(gam(value ~ track2, data=subset(rr.taxpc, variable=="taxpc2"))), colour=wes_palette("Moonrise3")[2], size = 4, parse=TRUE) +
