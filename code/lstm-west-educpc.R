...
west.educpc.lstm.pred.treated <- read_csv(paste0(results.directory, "lstm/west-educpc/treated-gans/weights.50-1.527.hdf5-west-educpc-test.csv"), col_names = FALSE)
west.educpc.lstm.pred.control <- read_csv(paste0(results.directory, "lstm/west-educpc/control/weights.210-33.268.hdf5-west-educpc-test.csv"), col_names = FALSE)
...
west.educpc.lstm.mse <- error(forecast=west.educpc.control.forecast, true=west.educpc.control.true, method = "mse") # post-intervention MSE
...
lstm.west.educpc.CI.treated <- PermutationCI(west.educpc.control.forecast, west.educpc.control.true, west.educpc.t.stat, west.educpc.n.placebo, c.range=c(-9,3), np=20000, l=1000)
...
mean(west.educpc.lstm.long$value[west.educpc.lstm.long$variable=="X1"])/mean(west.educpc.y[(west.educpc.n.pre+1):nrow(west.educpc.y),]) # get mean % treatment effect
mean(west.educpc.lstm.long$ymin[west.educpc.lstm.long$variable=="X1"])/mean(west.educpc.y[(west.educpc.n.pre+1):nrow(west.educpc.y),])
mean(west.educpc.lstm.long$ymax[west.educpc.lstm.long$variable=="X1"])/mean(west.educpc.y[(west.educpc.n.pre+1):nrow(west.educpc.y),])
