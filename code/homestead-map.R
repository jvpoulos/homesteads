...
data.directory <-"/homesteads/data/"
...
county.map$STATE_TERR <- droplevels(as.factor(county.map$STATE_TERR))
...
state_map$state.land <- ifelse(state_map$state_code %in% state.land.states, 1, 0)
...
homestead.70 <- ggplot(county.70, aes(long, lat, group = group, fill = homesteads.pc)) + geom_polygon() + 
  coord_equal()  + scale_fill_gradient(low = "#ffcccc", high = "#ff0000", na.value = "white") + labs(fill="Log per-capita\n cumulative\n homesteads\n in 1870\n") + 
...
# ggtitle("Log per-capita cumulative homesteads in 1870") +
 theme(plot.title = element_text(hjust = 0.5)) +
  geom_map(aes(map_id = id, colour = 'black'), map = county.f) + scale_colour_manual(values=c('black'),guide=FALSE) + theme(legend.position="right") +
geom_path( data = state_map[state_map$western.pub==1,] , colour = "yellow", size=1) +
  geom_path( data = state_map[state_map$southern.pub==1,] , colour = "orange", size=1) +
  geom_path( data = state_map[state_map$state.land==1,] , colour = "blue", size=1)
...
if(patient){
  for(t in sort(unique(patents.d$year2))){
    ...
    homestead.t <- ggplot(county.t, aes(long, lat, group = group, fill = homesteads.pc)) + geom_polygon() + 
      coord_equal()  + scale_fill_gradient(low = "#ffcccc", high = "#ff0000", na.value = "white") + labs(fill=paste("Log per-capita\n cumulative\n homesteads\n in",t,"\n")) + 
      theme(axis.ticks = element_blank(), axis.text.x = element_blank(), 
            axis.text.y = element_blank(),axis.title.x = element_blank(),
            axis.title.y = element_blank()) + theme(panel.grid.minor=element_blank(), 
                                                    panel.grid.major=element_blank()) +
