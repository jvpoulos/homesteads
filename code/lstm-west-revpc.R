...
west.revpc.lstm.pred.treated <- read_csv(paste0(results.directory, "lstm/west-revpc/treated-gans/weights.1970-6.458.hdf5-west-revpc-test.csv"), col_names = FALSE)
west.revpc.lstm.pred.control <- read_csv(paste0(results.directory, "lstm/west-revpc/control/weights.1340-4.422.hdf5-west-revpc-test.csv"), col_names = FALSE)
...
west.revpc.lstm.mse <- mean((west.revpc.control.true-west.revpc.control.forecast)**2) # post-intervention MSE
west.revpc.lstm.mse
...
west.revpc.lstm.long$ymin[west.revpc.lstm.long$group=="Treated"] <- west.revpc.CI.treated[,1]
west.revpc.lstm.long$ymax[west.revpc.lstm.long$group=="Treated"] <- west.revpc.CI.treated[,2]
...
mean(west.revpc.lstm.long$value[west.revpc.lstm.long$variable=="X1"])# get mean % treatment effect
mean(west.revpc.lstm.long$ymin[west.revpc.lstm.long$variable=="X1"])
mean(west.revpc.lstm.long$ymax[west.revpc.lstm.long$variable=="X1"])
