...
iid.placebo <- foreach(tau = taus) %dopar% {
       ...
       mclapply(capacity.outcomes.list, 
                ChernoCI, t_star=t_star_placebo, c.range=c(-2,2), sd=1, alpha=0.025, l=1000, prec=1e-02, ns=200, treated.indices=pub.states, permtype="iid",t0=t0_placebo,sim=FALSE,covars=NULL,pca=FALSE,mc.cores=cores)}
...
moving.block.placebo <- foreach(tau = taus) %dopar% {
  ...
  t_star_placebo <- t_final_placebo-t0_placebo
  mclapply(capacity.outcomes.list, 
           ChernoCI, t_star=t_star_placebo, c.range=c(-2,2), sd=1, alpha=0.025, l=1000, prec=1e-02, ns=200, treated.indices=pub.states, permtype="moving.block",t0=t0_placebo,sim=FALSE,covars=NULL,pca=FALSE,mc.cores=cores)}
...
iid.block.placebo <- foreach(tau = taus) %dopar% {
  ...
  t_star_placebo <- t_final_placebo-t0_placebo
  mclapply(capacity.outcomes.list, 
           ChernoCI, t_star=t_star_placebo, c.range=c(-2,2), sd=1, alpha=0.025, l=1000, prec=1e-02, ns=200, treated.indices=pub.states, permtype="iid.block",t0=t0_placebo,sim=FALSE,covars=NULL,pca=FALSE,mc.cores=cores)}
