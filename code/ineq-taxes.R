############################################
# State capacity vs. inequality plots #
############################################
...
ineq.capacity <- ggplot(ineq.taxpc, aes(aland.gini, value, color = factor(variable))) + 
  ...
  annotate("text", x = 0.6, y = 4, label = LmEq(gam(value ~ aland.gini, data=subset(ineq.taxpc, variable=="taxpc1"))), colour=wes_palette("Darjeeling")[4], size = 4, parse=TRUE) +
  geom_smooth(data=subset(ineq.taxpc, variable=="taxpc2"),se=TRUE, colour=wes_palette("Darjeeling")[5],size=1) +
  annotate("text", x = 0.6, y = -4, label = LmEq(gam(value ~ aland.gini, data=subset(ineq.taxpc, variable=="taxpc2"))), colour=wes_palette("Darjeeling")[5], size = 4, parse=TRUE) +
