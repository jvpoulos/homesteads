...
south.revpc.lstm.pred.treated <- read_csv(paste0(results.directory, "lstm/south-revpc/treated-gans/weights.1740-1.446.hdf5-south-revpc-test.csv"), col_names = FALSE)
south.revpc.lstm.pred.control <- read_csv(paste0(results.directory, "lstm/south-revpc/control/weights.1910-4.987.hdf5-south-revpc-test.csv"), col_names = FALSE)
...
south.revpc.lstm.mse <- mean((south.revpc.control.true-south.revpc.control.forecast)**2) # post-intervention MSE
...
south.revpc.lstm.long$ymin[south.revpc.lstm.long$group=="Treated"] <- south.revpc.CI.treated[,1]
south.revpc.lstm.long$ymax[south.revpc.lstm.long$group=="Treated"] <- south.revpc.CI.treated[,2]
...
mean(south.revpc.lstm.long$value[south.revpc.lstm.long$variable=="X1"])
mean(south.revpc.lstm.long$ymin[south.revpc.lstm.long$variable=="X1"])
mean(south.revpc.lstm.long$ymax[south.revpc.lstm.long$variable=="X1"])
