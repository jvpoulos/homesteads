...
moving.block <- mclapply(capacity.outcomes.list, 
           ChernoCI, t_star=t_star, c.range=c(-10,10), alpha=0.025, l=100, prec=1e-02, ns=100, treated.indices=pub.states, permtype="moving.block",t0=t0,sim=FALSE,covars=NULL,pca=FALSE,mc.cores=cores)
saveRDS(moving.block,"moving_block.rds")

iid.block <- mclapply(capacity.outcomes.list, 
           ChernoCI, t_star=t_star, c.range=c(-10,10), alpha=0.025, l=100, prec=1e-02, ns=100, treated.indices=pub.states, permtype="iid.block",t0=t0,sim=FALSE,covars=NULL,pca=FALSE,mc.cores=cores)
